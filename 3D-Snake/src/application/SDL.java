...
public class SDL {
    ...
    public SDL() {
        ...
        if (resultCode != 0) {
            ...
            error = SDL_GetError();
            throw new IllegalArgumentException(error + "\n'Couldn't initialize everything'\n~ Bozo the Clown");
        }
        ...
        window = SDL_CreateWindow("3D Snake", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width, height,
        ...
        glAttributes += SDL_GL_SetAttribute(SDL_GL_DEPTH_SIZE, 16); // overlapping graphics
        glAttributes += SDL_GL_SetAttribute(SDL_GL_DOUBLEBUFFER, 1);
        if (glAttributes != 0) {
            error = SDL_GetError();
            throw new IllegalArgumentException(error
                    + "\n'OpenGL used Clownin Around!'\n'It was super effective!'\n(OpenGl attributes had an error)");
        }
